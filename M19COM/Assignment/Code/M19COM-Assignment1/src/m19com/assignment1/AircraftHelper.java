...
public class AircraftHelper {
    ...
    public static Aircraft genAircraft() {
        ...
        String currenttype = types.get((int)( 2 * Math.random()));

        switch (currenttype) {
            case "JET":
                return new PropellerAircraft(name, flights, reg,currenttype);
            case "PROP":
                return new JetAircraft(name, flights, reg,currenttype);
            default:
